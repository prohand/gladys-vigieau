...
test('normalizeConfig returns the defaults when called with no argument', () => {
  assert.deepEqual(normalizeConfig(), { ...DEFAULT_CONFIG, locations: [] });
});
...
test('the coordinates stored as numbers by <= 1.1.1 still migrate', () => {
  // They were config fields then, declared `number`. They are no longer in the
  // config_schema at all, so nothing sends them back and nothing has to rewrite
  // them — they only have to be READ once, whatever their JSON type.
