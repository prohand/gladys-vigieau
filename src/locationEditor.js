...
//
// WHAT THE USER SEES. One dropdown ("Sélectionner un lieu", an action form)
// points the screen at one location of the list; the four config fields below
// — name, address, latitude, longitude — MIRROR that location, so its
// information is displayed, editable, and saved back by the ordinary Save
// button. "Supprimer le lieu sélectionné" removes the same one. Adding stays
// what it always was: type an address, the integration geocodes it.
//
// WHY THE DROPDOWN IS AN ACTION AND NOT A CONFIG FIELD. A `select` in a
// manifest has STATIC options (`externalIntegration.validateConfigValue` reads
// the valid values straight from the manifest, and the only dynamic source the
// core defines, `devices`, has no server-side implementation in any released
// Gladys — up to 4.84.4 every value such a dropdown offers is refused with a
// 422 before the command reaches this container). So the options can only be
// POSITIONS in the list, and the `lieux` field is what maps a position to a
// name. An action was chosen over a config field for one reason: the core
// displays an action's result message under its button, and displays NOTHING
// of what an integration has to say about a Save — `setConnectionStatus` is
// rendered on the Supervision page, not here. Selecting a location has to be
// able to answer "Lieu 2 « Jardin », reload the page to edit it".
//
...
export function createLocationEditor({
   ...
   * @param {string} params.selectedId - the location to point the screen at
   ...
   */
  async function commit({ locations, selectedId, warning = '', republish = true }) {
    const selected = findLocationById(locations, selectedId) ?? locations[0] ?? null;
    ...
    const summary = describeLocations(locations, id);
    await setConfig({
  ...
  return {
     ...
     */
    async sync() {
      const { locations, selectedId } = getConfig();
      await commit({ locations, selectedId, republish: false });
    },

    /**
     * Apply what the user just typed in the detail fields to the location the
     * screen is pointed at. Called on every `config-updated`.
     *
     ...
     * an integration says about a Save — so it is logged, and a problem is
     * carried into the `lieux` field, which the next page load shows.
     * @returns {Promise<{ en: string, fr: string } | null>}
     */
    async applyFormEdits() {
      const config = getConfig();
      const { locations, selectedId } = config;
      const incoming = canonical(readDetailFields(config));

      if (locations.length === 0) {
        // Nothing to mirror. The detail fields are then just an empty form:
        // adding a location is what the search action is for, and creating one
        // from here would resurrect the location the user has just deleted,
        // whose values the open form is still showing.
        staleFields = null;
        return null;
      }

      const current = findLocationById(locations, selectedId);
      const stored = canonical(detailFieldsOf(current));
      ...
      if (sameFields(fields, stored)) {
        if (!sameFields(incoming, stored)) {
          // The Save was neutralized, but the core has already written what the
          // form sent into the mirror fields — and the tab is still showing it,
          // so it would send exactly the same thing again. Rewriting them puts
          // the database back in step with the selected location and re-arms
          // the guard on what the screen now displays; without this, the second
          // Save of an unreloaded page applies the previous location's point.
          await commit({ locations, selectedId: current.id, republish: false });
        }
        return null;
      }

      const patch = { id: current.id };
      // Collected rather than assigned: the name and the point are edited in
      // the same Save, and the first thing that went wrong is the one shown.
      const problems = [];
      ...
      if (coordinatesEdited && latitude !== null && longitude !== null) {
        ...
        // The address is the natural way to move a location: geocode it and
        // let the coordinates follow.
      ...
      await commit({
        ...
        warning: problem ? `⚠ ${problem.fr}` : '',
      });

      const saved = findLocationById(getConfig().locations, current.id);
      ...
      logger.info(`Detail fields applied to "${saved.name}": ${describeLocation(saved)}`);
      return message;
    },
    ...
    actions: {
      ...
      async rechercher_adresse(fields = {}) {
        ...
        await commit({
          locations: upsertLocation(locations, { id, name, ...point }),
          selectedId: id,
        });
        ...
        return {
          en: `Location ${position} "${name}" added and selected: ${describeLocation(saved)}. Add its device from the Discovery tab, and reload this page (F5) to edit it.`,
          fr: `Lieu ${position} « ${name} » ajouté et sélectionné : ${describeLocation(saved)}. Ajoutez son appareil depuis l’onglet Découverte, et rechargez cette page (F5) pour le modifier.`,
        };
      },

      /**
       * Point the Configuration screen at another location: its information
       * goes into the fields above, where it can be edited and saved.
       */
      async selectionner_lieu(fields = {}) {
        logger.info(`Action selectionner_lieu <- ${fields.lieu ?? ''}`);
        const { locations } = getConfig();
        if (locations.length === 0) {
        ...
        await commit({ locations, selectedId: location.id });
        return {
          en: `Location ${fields.lieu} selected: ${describeLocation(location)}. Reload this page (F5) to see and edit it in the fields above.`,
          fr: `Lieu ${fields.lieu} sélectionné : ${describeLocation(location)}. Rechargez cette page (F5) pour le voir et le modifier dans les champs ci-dessus.`,
        };
      },

      /** Remove the selected location from the list. */
      async supprimer_lieu(fields = {}) {
        logger.info(`Action supprimer_lieu <- confirmation=${fields.confirmation ?? false}`);
        const { locations, selectedId } = getConfig();
        const location = findLocationById(locations, selectedId);
        if (!location) {
          return noLocationMessage();
        }
        if (fields.confirmation !== true) {
          ...
          return {
            en: `Tick "I confirm" to delete "${location.name}" (${describeLocation(location)}).`,
            fr: `Cochez « Je confirme » pour supprimer « ${location.name} » (${describeLocation(location)}).`,
          };
        }

        const remaining = removeLocation(locations, location.id);
        await commit({ locations: remaining, selectedId: remaining[0]?.id ?? '' });
        ...
        const next = remaining.length
          ? {
              en: ` "${remaining[0].name}" is now selected — reload this page (F5).`,
              fr: ` « ${remaining[0].name} » est maintenant sélectionné — rechargez cette page (F5).`,
            }
          : { en: '', fr: '' };
        return {
          en: `Location "${location.name}" removed. Delete its device in Gladys too: an integration cannot delete it for you.${next.en}`,
          fr: `Lieu « ${location.name} » supprimé. Supprimez aussi son appareil dans Gladys : une intégration ne peut pas le faire à votre place.${next.fr}`,
        };
      },
    },
  };
